...
func uninstallPkg(ctx context.Context, pi goolib.PackageInfo, state *client.GooGetState, dbOnly bool, proxyServer string, insecure bool) error {
...
func All(ctx context.Context, pi goolib.PackageInfo, deps DepMap, state *client.GooGetState, dbOnly bool, proxyServer string, insecure bool) error {
	for len(deps) > 1 {
		for dep := range deps {
			if len(deps[dep]) == 0 {
				di := goolib.PkgNameSplit(dep)
				if err := uninstallPkg(ctx, di, state, dbOnly, proxyServer, insecure); err != nil {
					return err
				}
				deps.remove(dep)
			}
	...
	return uninstallPkg(ctx, pi, state, dbOnly, proxyServer, insecure)
}
